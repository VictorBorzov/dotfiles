...
"custom/memory" = {
  ...
  return-type = "json";
  "on-click" = "foot btm";
  exec = pkgs.writeShellScript "memory-json" ''
    summary=$(free -m | awk '/Mem:/ { printf("🧬 %.1fGB/%.1fGB ", $3/1024, $2/1024) } ')
    tooltip=$(ps -eo rss,comm --sort=-rss \
      | awk 'NR>1 && NR<=11 { printf "%s %.1fMB\n", $2, $1/1024 }')

    # Proper JSON escaping (quotes, backslashes, newlines)
    tooltip_json=$(printf '%s' "$tooltip" \
      | sed ':a;N;$!ba;s/\\/\\\\/g;s/"/\\"/g;s/\n/\\n/g')

    echo "{\"text\": \"''${summary}\", \"tooltip\": \"''${tooltip_json}\"}"
  '';
};
"custom/cpu" = {
  interval = 5;
  return-type = "json";
  "on-click" = "foot btm";
exec = pkgs.writeShellScript "cpu-json" ''
    cores=$(nproc)
    snapshot=$(top -b -n2 -d0.2)

    # Get total CPU usage from second snapshot (accurate total)
    usage=$(echo "$snapshot" \
      | awk '/^%Cpu/ {cpu=$0} END {split(cpu,a,","); printf("⚙️ %.1f%% ", 100 - a[4])}')

    # Get top 10 processes CPU% from second snapshot, normalized per core
    tooltip=$(echo "$snapshot" \
      | awk -v cores="$cores" 'f && /^[ ]*[0-9]/ {
          cpu = $9/cores;
          if(cpu > 0.1) printf "%s %.1f%%\n", $12, cpu
        }
        /^ *PID/{f=1}' \
      | sort -k2 -nr 2>/dev/null \
      | head -n10)
    ...
    echo "{\"text\": \"''${usage}\", \"tooltip\": \"''${tooltip_json}\"}"
  '';
};
...
' & disown";
        ...
        "custom/weather" = {
          "format" = "🌦";
          "on-click" = "foot --hold -e wthrr belgrade -f d";
          "tooltip" = true;
        };
        "custom/powermenu" = {
          "format" = "";
