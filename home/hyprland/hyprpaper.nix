...
in {
  ...
  services.hyprpaper = {
    ...
    settings = {
      preload = [
        (if self.theme.dark then w1Dark else w1Light)
        (if self.theme.dark then w2Dark else w2Light)
      ];
      wallpaper = [
        "eDP-1,${if self.theme.dark then w1Dark else w1Light}"
        "HDMI-A-1,${if self.theme.dark then w2Dark else w2Light}"
      ];
    };
  };
}
